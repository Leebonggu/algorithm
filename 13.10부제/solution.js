...
자동차 10부제는 자동차 번호의 일의 자리 숫자와 날짜의 일의 자리 숫자가 일치하면해당 자동차의 운행을 금지하는 것이다.
...
function solution(target, numbers) {
  return numbers.filter((number) => String(number)[1] === String(target)).length;
}

console.log(solution(3, [25, 23, 11, 47, 53, 17, 32]));
